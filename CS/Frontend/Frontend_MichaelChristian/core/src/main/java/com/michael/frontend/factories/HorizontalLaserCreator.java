...
import com.michael.frontend.obstacles.HorizontalLaser;
import com.michael.frontend.pools.HorizontalLaserPool;
...
public class HorizontalLaserCreator implements ObstacleFactory.ObstacleCreator {
    ...
    public BaseObstacle create(float groundTopY, float spawnX, float playerHeight, Random rng) {
        float length = rng.nextFloat() * (MAX_LENGTH - MIN_LENGTH) + MIN_LENGTH;
