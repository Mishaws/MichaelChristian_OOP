...
public class Main extends Game {
    ...
    private void renderGame(ShapeRenderer shapeRenderer) {
        ScreenUtils.clear(0, 0, 0, 1); // Bersihkan layar

        shapeRenderer.setProjectionMatrix(camera.combined);
        shapeRenderer.begin(ShapeRenderer.ShapeType.Filled);

        // Render ground dan player (menggunakan method renderShape() dari kelas Anda)
    ...
    private void spawnObstacle() {
        float cameraRightEdge = camera.position.x + (camera.viewportWidth / 2);
